...
- [`07b6-playtest-loop.md`](./07b6-playtest-loop.md) — `done`. Repeatable M0 playtest checklist and smoke path. *Start learning what is fun.*
...
- **Gameplay command surface.** Use rough dev controls for move, Move Only, target, halt-on-sight, build, train, research, cancel, and abilities so the full M0 roster can be exercised.
...
- [x] Dev can drive all non-ability M0 orders from the UI: move, Move Only, target, halt-on-sight, build, train, research, and cancel. *(07b4)*
