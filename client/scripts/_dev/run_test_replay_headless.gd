...
const TEST_SCRIPT := "res://scripts/_dev/test_replay.gd"


func _init() -> void:
	var script: Script = load(TEST_SCRIPT) as Script
	if script == null:
		push_error("[test_replay_headless] could not load %s" % TEST_SCRIPT)
		quit(1)
		return
	var runner: Node = Node.new()
	runner.set_script(script)
	if not runner.has_method("_run_all"):
		push_error("[test_replay_headless] test runner did not expose _run_all")
		quit(1)
		return
