...
func _init() -> void:
	...
	_run_idle_out_of_range(runner)
	_run_idle_in_auto_attack_range(runner)
	_run_attack_orders_only(runner)
...
func _run_idle_out_of_range(runner: Node) -> void:
	...
	_run_case("idle_out_of_range", fixture["state"], SubmitTurn.new(), SubmitTurn.new(), registry)


func _run_idle_in_auto_attack_range(runner: Node) -> void:
	var fixture: Dictionary = runner._build_stress_fixture()
	_run_case(
...
func _run_attack_orders_only(runner: Node) -> void:
	var fixture: Dictionary = runner._build_stress_fixture()
	...
	_run_case("attack_orders_only", fixture["state"], submit_a, submit_b, fixture["registry"])
