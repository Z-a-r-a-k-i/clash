...
func _all_tests() -> Array:
	return [
		...
		["initiative_lethal_units_both_fire", _test_initiative_lethal_units_both_fire],
		[
			"initiative_kill_prevents_later_normal_attack",
			_test_initiative_kill_prevents_later_normal_attack
		],
		...
		[
			...
			_test_move_to_enemy_occupied_tile_fires_and_moves_to_nearest_open
		],
		[
			"move_blocked_adjacent_target_completes_at_nearest_open",
		...
		[
			"ground_and_flying_movers_can_share_destination",
			_test_ground_and_flying_movers_can_share_destination
		],
		["flying_blocker_blocks_flying", _test_flying_blocker_blocks_flying],
		["failed_mover_remains_blocker", _test_failed_mover_remains_blocker],
		["gather_travel_worker_is_passable", _test_gather_travel_worker_is_passable],
		["construction_travel_worker_is_passable", _test_construction_travel_worker_is_passable],
		[
		...
		[
			"target_generated_move_reaches_weapon_range",
			_test_target_generated_move_reaches_weapon_range
		],
		[
			"target_generated_move_keeps_last_known_tile_without_target",
		...
		[
			"post_movement_only_attacks_after_moving_into_range",
			_test_post_movement_only_attacks_after_moving_into_range
		],
		[
			"both_window_unit_attacks_once_per_window",
			_test_both_window_unit_attacks_once_per_window
		],
		[
			"post_movement_attack_does_not_reduce_prior_movement_budget",
			_test_post_movement_attack_does_not_reduce_prior_movement_budget
		],
		["move_shoots_before_moving", _test_move_shoots_before_moving],
		...
		["order_builder_fan_out_target", _test_order_builder_fan_out_target],
		...
		["far_gather_rally_target_rejected", _test_far_gather_rally_target_rejected],
...
func _test_initiative_kill_prevents_later_normal_attack() -> bool:
	...
	for ev in result.events:
		if (
			ev.type == ResolverEvent.Type.ENTITY_DAMAGED
			and ev.actor_id == initiative_unit.id
			and ev.target_id == normal_unit.id
		):
			initiative_hit = true
		if (
			ev.type == ResolverEvent.Type.ENTITY_DAMAGED
			and ev.actor_id == normal_unit.id
	...
	return (
		initiative_hit
		and not normal_hit
		and new_initiative.current_hp == 50
		and new_normal.current_hp == 0
	)
...
func _test_multi_tile_move_collision() -> bool:
	# 2x2 mover at (1,1)-(2,2). 2x2 blocker at (3,1)-(4,2). A* may route
	# around the blocker, but the committed footprint must never overlap it.
	...
	var new_mover := result.new_state.get_entity_by_id(mover.id)
	var new_blocker := result.new_state.get_entity_by_id(blocker.id)
	var mover_rect: Rect2i = result.new_state.tile_grid.entity_rect(new_mover.id)
	var blocker_rect: Rect2i = result.new_state.tile_grid.entity_rect(new_blocker.id)
	return new_mover.origin != mover.origin and not mover_rect.intersects(blocker_rect)
...
func _test_move_routes_around_static_blocker() -> bool:
	...
	for ev in result.events:
		if (
			...
			and ev.to_origin == blocker.origin
		):
			push_error("mover should not step through a static blocker")
			return false
	return true
	...
	var result: ResolveResult = (
		...
		. resolve(
			...
			_submit(
				(
					[
						_move_order(rear.id, EntityOrder.Type.MOVE, Vector2i(3, 1)),
						_move_order(front.id, EntityOrder.Type.MOVE, Vector2i(4, 1)),
					]
					as Array[EntityOrder]
				)
			),
			_submit(),
			registry,
	...
	var result: ResolveResult = (
		...
		. resolve(
			...
			_submit(
				(
					[
						_move_order(left.id, EntityOrder.Type.MOVE, Vector2i(2, 1)),
						_move_order(right.id, EntityOrder.Type.MOVE, Vector2i(1, 1)),
					]
					as Array[EntityOrder]
				)
			),
		...
		. resolve(
			...
			_submit(
				(
					[
						_move_order(left.id, EntityOrder.Type.MOVE, Vector2i(3, 1)),
						_move_order(right.id, EntityOrder.Type.MOVE, Vector2i(3, 1)),
					]
					as Array[EntityOrder]
				)
			),
...
func _test_movement_respects_impassable_terrain_tags() -> bool:
	...
	for ev in result.events:
		if (
			ev.type == ResolverEvent.Type.ENTITY_MOVED
			and ev.actor_id == actor.id
			and ev.to_origin == Vector2i(2, 1)
		):
			push_error("mover should not step onto impassable terrain")
			return false
	return true
...
func _test_move_to_enemy_occupied_tile_fires_and_moves_to_nearest_open() -> bool:
	var registry: EntityRegistry = _combat_mover_registry(6, 10, 6)
	var state: MatchState = _state_with_grid(8, 3)
	var actor: Entity = _make_entity(state, "marine", 0, Vector2i(1, 1), 50, "ground")
	var target: Entity = _make_entity(state, "marine", 1, Vector2i(5, 1), 50, "ground")
	state.tile_grid.place(actor.id, Rect2i(1, 1, 1, 1))
	state.tile_grid.place(target.id, Rect2i(5, 1, 1, 1))

	var result: ResolveResult = Resolver.resolve(
		state,
		_submit([_move_order(actor.id, EntityOrder.Type.MOVE, target.origin)]),
		_submit(),
		registry,
		null
	)
	...
	var result: ResolveResult = (
		...
		. resolve(
			...
			_submit(
				(
					[
						_move_order(ground_actor.id, EntityOrder.Type.MOVE, Vector2i(2, 1)),
						_move_order(flyer_actor.id, EntityOrder.Type.MOVE, Vector2i(2, 1)),
					]
					as Array[EntityOrder]
				)
			),
	...
	var result: ResolveResult = (
		...
		. resolve(
			...
			_submit(
				(
					[
						_move_order(rear.id, EntityOrder.Type.MOVE, Vector2i(2, 1)),
						_move_order(stuck.id, EntityOrder.Type.MOVE, Vector2i(4, 1)),
					]
					as Array[EntityOrder]
				)
			),
...
func _test_move_and_auto_attack_resolve_independently() -> bool:
	# Attack-ground fires before movement when an enemy starts in range and
	# then halts instead of spending post-shot movement.
...
func _test_move_shoots_before_moving() -> bool:
	...
	move_order.target_tile = Vector2i(12, 5)
...
func _test_attack_move_does_not_fire_after_moving_into_range() -> bool:
	var registry := _combat_mover_registry(6, 3, 4)
	var state := _state_with_grid(20, 20)
	var actor := _make_entity(state, "marine", 0, Vector2i(5, 5), 50, "ground")
...
func _test_both_window_unit_attacks_once_per_window() -> bool:
	...
	return (
		...
		and moved_idx > first_damage_idx
		and second_damage_idx > moved_idx
		and new_actor.origin == Vector2i(5, 6)
	)
...
func _test_latest_move_like_intent_wins() -> bool:
	...
	move_only.type = EntityOrder.Type.MOVE
...
func _test_target_generated_move_reaches_weapon_range() -> bool:
	var registry: EntityRegistry = _combat_mover_registry(6, 3, 4)
	var state: MatchState = _state_with_grid(20, 20)
	var actor: Entity = _make_entity(state, "marine", 0, Vector2i(1, 1), 50, "ground")
	...
	move.type = EntityOrder.Type.MOVE
	move.entity_id = actor.id
	move.target_entity_id = target.id
	move.target_tile = Vector2i(5, 1)
	var attack: EntityOrder = EntityOrder.new()
	attack.type = EntityOrder.Type.TARGET
	attack.entity_id = actor.id
	attack.target_entity_id = target.id
	attack.target_priority_chain = [target.id]
	attack.target_tile = target.origin

	var result: ResolveResult = Resolver.resolve(
		state, _submit([move, attack] as Array[EntityOrder]), _submit(), registry, null
	)
...
func _test_target_generated_move_keeps_last_known_tile_without_target() -> bool:
	var registry: EntityRegistry = _combat_mover_registry(6, 3, 4)
	var state: MatchState = _state_with_grid(20, 20)
	var actor: Entity = _make_entity(state, "marine", 0, Vector2i(1, 1), 50, "ground")
	state.tile_grid.place(actor.id, Rect2i(1, 1, 1, 1))
	...
	var new_actor: Entity = result.new_state.get_entity_by_id(actor.id)
	return new_actor.origin == Vector2i(5, 1) and new_actor.focus_target_entity_id == -1


func _test_target_rejects_unattackable_layer() -> bool:
	var registry: EntityRegistry = _combat_mover_registry(6, 3, 4)
	var state: MatchState = _state_with_grid(20, 20)
	var actor: Entity = _make_entity(state, "marine", 0, Vector2i(1, 1), 50, "ground")
	...
	if not _has_rejection(result.events, actor.id, "bad_target"):
		push_error("target on unattackable layer should reject before queueing")
		return false
	for ev in result.events:
		if ev.type == ResolverEvent.Type.ENTITY_MOVED and ev.actor_id == actor.id:
			push_error("rejected target should not move")
			return false
		if ev.type == ResolverEvent.Type.ENTITY_DAMAGED and ev.actor_id == actor.id:
			push_error("rejected target should not deal damage")
			return false
	var new_actor: Entity = result.new_state.get_entity_by_id(actor.id)
...
func _test_order_builder_fan_out_attack_move() -> bool:
	...
	var orders := OrderBuilder.fan_out_attack_move(ids, Vector2i(8, 8))
	if orders.size() != 2:
		return false
	for i in 2:
		var o: EntityOrder = orders[i]
...
func _test_order_builder_fan_out_target() -> bool:
...
func _test_far_gather_rally_target_rejected() -> bool:
	var registry: EntityRegistry = _gather_registry(5, 1, 4)
	var base_def: EntityDef = registry.get_by_id("base")
	if base_def == null:
		push_error("gather registry should define a base")
		return false
	base_def.production = ProductionDef.new()
	base_def.production.produces = ["worker"]
	var state: MatchState = _state_with_grid(40, 20)
	state.players[0].minerals = 500
	state.players[0].pop_cap = 20
	var base: Entity = _make_entity(state, "base", 0, Vector2i(1, 8), 1500, "ground")
	base.production_state = ProductionState.new()
	state.tile_grid.place(base.id, Rect2i(1, 8, 4, 4))
	var far_patch: Entity = _make_entity(state, "minpatch", -1, Vector2i(30, 8), 0, "ground")
	far_patch.current_resource_amount = 500
	state.tile_grid.place(far_patch.id, Rect2i(30, 8, 1, 1))
	_add_opponent_keepalive_building(state, registry)

	var order := EntityOrder.new()
	order.type = EntityOrder.Type.SET_RALLY_POINT
	order.entity_id = base.id
	order.mode = ProductionState.RALLY_MODE_GATHER
	order.target_entity_id = far_patch.id
	var result := Resolver.resolve(state, _submit([order]), _submit(), registry, null)
	var new_base: Entity = result.new_state.get_entity_by_id(base.id)
	if not _has_rejection(result.events, base.id, "bad_rally_gather_target"):
		push_error("far gather rally should be rejected during resolver validation")
		return false
	return (
		new_base != null
		and new_base.production_state != null
		and new_base.production_state.rally_mode == ProductionState.RALLY_MODE_NONE
	)
...
func _test_train_ground_spawn_deferred_when_ring_is_cliff() -> bool:
	...
	if not _has_event_of_type(result.events, ResolverEvent.Type.SPAWN_DEFERRED):
		...
		return false
	var updated_barracks := result.new_state.get_entity_by_id(barracks.id)
...
func _test_train_flying_spawn_allows_cliff_tiles() -> bool:
	...
	var spawn := ProductionSystem.find_spawn_tile(state, registry, barracks, flyer_def)
	if spawn == Vector2i(-1, -1):
...
func _test_match_state_save_load_roundtrip() -> bool:
	...
	queued.type = EntityOrder.Type.TARGET
...
func _move_order(entity_id: int, type: EntityOrder.Type, target_tile: Vector2i) -> EntityOrder:
	var order: EntityOrder = EntityOrder.new()
	order.type = type
	order.entity_id = entity_id
...
func _mark_spawn_ring_terrain(
	...
	var top := producer_rect.position.y - unit_fp.y
	var bottom := producer_rect.position.y + producer_rect.size.y
	var left := producer_rect.position.x - unit_fp.x
	var right := producer_rect.position.x + producer_rect.size.x
	for x in range(producer_rect.position.x, producer_rect.position.x + producer_rect.size.x):
		_mark_spawn_candidate_terrain(state, Vector2i(x, top), unit_fp, tags)
	for y in range(producer_rect.position.y, producer_rect.position.y + producer_rect.size.y):
...
func _mark_spawn_candidate_terrain(
	...
	for x in range(origin.x, origin.x + unit_fp.x):
		for y in range(origin.y, origin.y + unit_fp.y):
...
func _set_combat_timing(
	combat: CombatDef,
	attacks_before_movement: bool,
	attacks_after_movement: bool,
	has_initiative: bool
) -> void:
	if combat == null:
		return
	combat.set("attacks_before_movement", attacks_before_movement)
	combat.set("attacks_after_movement", attacks_after_movement)
	combat.set("has_initiative", has_initiative)
