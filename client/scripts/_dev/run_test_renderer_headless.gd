...
const TEST_SCRIPT := "res://scripts/_dev/test_renderer.gd"


func _init() -> void:
	var script: Script = load(TEST_SCRIPT) as Script
	if script == null:
		push_error("[test_renderer_headless] could not load %s" % TEST_SCRIPT)
		quit(1)
		return
	...
	print("[test_renderer_headless] %d passed, %d failed" % [passed, failed])
