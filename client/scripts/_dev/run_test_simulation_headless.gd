...
const TEST_SCRIPT := "res://scripts/_dev/test_simulation.gd"


func _init() -> void:
	var script: Script = load(TEST_SCRIPT) as Script
	if script == null:
		push_error("[test_simulation] could not load %s" % TEST_SCRIPT)
		quit(1)
		return
	...
	print("[test_simulation] %d passed, %d failed" % [passed, failed])
